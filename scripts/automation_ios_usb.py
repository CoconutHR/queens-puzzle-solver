"""Queens 自动解题（USB 隧道版）：截图 → analyze → 点击。

用法：
    python automation_ios.py
    python automation_ios.py --udid 00008120-XXXXXXXXXXXXXXXX        # 多设备时锁定目标
    python automation_ios.py --local-port 9097 --no-logs             # 端口冲突时避让
    python automation_ios.py --dry-run                               # 只识别，不点击

前置条件：
    * iproxy 已安装（本机 /opt/homebrew/bin/iproxy），可用 `asc doctor` 确认；
    * iPhone 与本机通过 USB 连接，已解锁并信任本机；
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="通过 USB 隧道识别 Queens 棋盘并自动落子")
    parser.add_argument("--config", help="asclient.json 路径，默认取当前目录")
    parser.add_argument(
        "--device",
        help="覆盖连接地址；USB 场景一般不需要（默认用隧道本地地址）",
    )
    ...
    try:
        ...
        print("排查：确认 iPhone 与本机通过 USB 连接、已解锁并信任本机，且 AScript 服务已启动。", file=sys.stderr)
        return 3
