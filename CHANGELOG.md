...
- **截图识别**（`io::image` 模块）：从 PNG / JPG / WebP 截图中提取 n×n 区域布局，
  ...
  1. **彩色 mask + 行列投影**（首选，适合带 UI 的完整手机截图）：格子是彩色，
     而白线、米色边距和 UI 文字都接近灰，据此生成 mask 再做行列投影定位棋盘与每格。
     含亮度保护、按图高自适应的缺口合并、变异系数校验、格子填充率校验与多特征加权打分
  2. **梯度剖面回退**（适合已裁剪好的棋盘，或格子颜色偏浅/pastel 导致策略 1 失效时）
  定位后对每格中心区域采样、忽略低色度像素，再按 **HSV 距离**聚类得到区域
  （HSV 阈值很关键：棋盘常用同色相不同明度的配色，阈值过大会误合并，取 0.08）
