...
- **截图识别**：从 PNG / JPG / WebP 截图中直接提取区域布局（基于颜色梯度检测，对网格线和
  边距颜色无要求 —— 白色、黑色、米色均可）。
...
截图识别采用两种互补策略，自动适配，无需手动圈选棋盘位置：

1. **饱和度 mask + 行列投影**（首选）：棋盘格子是彩色，而白线、米色边距和 UI
   文字都是低饱和度，据此生成 mask 后做行列投影即可定位棋盘与每个格子。
   这条路径对**带 UI 的完整手机截图**很稳健。
2. **梯度剖面回退**：用相邻像素的色差定位网格线。当棋盘格子颜色偏浅（pastel，
   饱和度很低）导致策略 1 失效时自动启用，适合**已裁剪好的棋盘截图**。

定位到格子后，对每格中心区域采样平均色，再按颜色聚类得到区域布局。
...
- **从棋盘截图识别皇后 / 叉**：当前截图识别只提取区域布局，识别已有皇后和 X 需要额外的视觉启发式。
- **截图导入**：识别粘贴的棋盘截图并提取区域布局（已实现，见 `cargo run -- solve xxx.png`）。
